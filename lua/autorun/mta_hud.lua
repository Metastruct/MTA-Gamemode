...
local CustomScale = 1
local ScrRatio = ScrH() / 1080 * CustomScale
...
hook.Add("HUDPaint", "SyntHud", function()
	...
	if player:Alive() then
		...
		local x, y, w, h = HealthBarPercentageStartW,
			HealthBarPercentageStartH - (45 * ScrRatio),
	...
	if Weapon:IsValid() then
		...
		ClipShakeH = math.Rand(-200 * ScrRatio, 200 * ScrRatio) * math.min(CurTime() - ClipShakeTime, 0)
		ClipShakeW = math.Rand(-200 * ScrRatio, 200 * ScrRatio) * math.min(CurTime() - ClipShakeTime, 0)
		BgClipShakeH = math.Rand(-100 * ScrRatio, 100 * ScrRatio) * math.min(CurTime() - BgClipShakeTime, 0)
		BgClipShakeW = math.Rand(-100 * ScrRatio, 100 * ScrRatio) * math.min(CurTime() - BgClipShakeTime, 0)
		...
		if Reloading == true and AmmoAnimStep == 0 then
			AmmoLocation = (44 * ScrRatio) * SequenceCycle
			AmmoFade = math.Rand(25, 75)
		end
		...
		if Ammo > 0 then
			draw.SimpleText(Ammo, "Font Ammo",
				HealthBarPercentageStartW + (10 * ScrRatio) + (ClipShakeW / 2),
				HealthBarPercentageStartH - (43 * ScrRatio) + (ClipShakeH / 2),
		...
		ClipLocationV = HealthBarPercentageStartH - (110 * ScrRatio)
		...
		if AmmoType > -1 then
			...
			draw.SimpleText(
				...
				ClipLocationV + (4 * ScrRatio) + ClipShakeH + BgClipShakeH,
			...
			if Clip >= 100 then
				draw.SimpleText(
					...
					ClipLocationV + (-17 * ScrRatio) + BgClipShakeH,
	...
	if player:Alive() then
		...
		if player.HpGainDifference > 0 then
			draw.SimpleText(
				...
				"Font Bars",
				HealthBarPercentageStartW + (4 * ScrRatio),
				HealthBarPercentageStartH + (-3 * ScrRatio),
		...
		if BarsDigits == true then
			draw.SimpleText(
				player:Health(),
				"Font Bars",
				HealthBarPercentageStartW + (22 * ScrRatio),
		...
		if MaxAp > 0 then
			...
			surface.SetDrawColor(244, 135, 2)
			surface.DrawTexturedRect(
				math.Clamp(ApStart, HealthBarPercentageStartW + (ArmorClamp * ScrRatio), 100000),
				HealthBarPercentageStartH + (23 * ScrRatio),
				18 * ScrRatio,
				18 * ScrRatio
			)
			...
			if BarsDigits == true then
				draw.SimpleText(
					player:Armor(),
					"Font Bars",
					math.Clamp(ApStart, HealthBarPercentageStartW + (ArmorClamp * ScrRatio), 100000) + (22 * ScrRatio),
					HealthBarPercentageStartH + (20 * ScrRatio),
